	...
	<body>
		...
		<form id="erregistratuF" method="POST" action="signUp.php" name="erregistratuF" enctype="multipart/form-data">
			<fieldset>
				...
				<label>Irakaslea zara?</label>
				<input type="checkbox" id="chbx_rola" name="chbx_rola" value = "irakaslea"/><br/><br/>
...
<?php
	if(isset($_POST['txt_eposta'])){
		...
		$argazkia = NULL;
		$rola = false;
		if(isset($_POST['chbx_rola'])){
			$rola = $_POST['chbx_rola'] == "irakaslea";
		}
		...
		$sql = "INSERT INTO users (eposta, deitura, goitizena, pasahitza, irudia, rola) 
				VALUES ('$eposta', '$deitura', '$goitizena', '$pasahitza', '{$argazkia}','$rola')";
